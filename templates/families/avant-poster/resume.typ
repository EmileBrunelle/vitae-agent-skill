...
#let accent2 = rgb("#1f3a5f")   // hue B — the dividers, the markers, icons, dates
...
#let muted = ink
#let dark = pal.dark
#let bodyf = ("Fira Sans", "Noto Sans", "Liberation Sans")
#let dispf = ("Chivo", "Big Shoulders Display", "Noto Sans")
...
#set par(justify: false, leading: 0.548em, spacing: 0.638em)
...
// page 1, the name oversized under it, the role line, the specialities line,
// then the contacts.
//
// NO DIAGONAL. An earlier version placed a rotated hue-B bar top right. It
// failed the § Anti-AI-slop test on the one question that matters — "does this
// element do work the reader can name?": readers asked what it was for and
// there was no answer. A diagonal that sets an axis is a decision; a diagonal
// that only signals "constructivist" is decorative geometric confetti with a
// manifesto. The character is carried by the two elements that DO work: the
// bleeding bars (they bound the sections and leave the text block) and the
// committed two-ink discipline.
//
// The masthead bar is `place`d, so it lands on page 1 only: a page
// `background` would repeat it on page 2 (verified on color-band).
...
// ONE line. `Tools & DevOps` wrapped; it is two rows now — more rows with
// fewer items each, which is the prescribed fix.
