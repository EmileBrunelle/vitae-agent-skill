// FAMILY: humanist-quiet — Vignelli-style restraint, no rules anywhere
...
#let muted = ink
#let dark = pal.dark
#let bodyf = ("Cantarell", "Noto Sans", "Liberation Sans")
#let dispf = bodyf
...
#set par(justify: false, leading: 0.545em, spacing: 0.645em)
#show link: set text(fill: accent)

// device: none — whitespace and a size/weight step do all the delimiting
#let section(t, body, airy: false) = block(breakable: false,
  above: if airy { 31pt } else { 26pt }, below: 0pt)[
...
// skills: bulleted rows, accent marker, em-dash label (hanging indent aligns wraps)
#let skill(cat, items) = srow([#text(fill: accent, weight: "bold")[–]#h(6pt)#text(weight: "semibold")[#cat] —], items, hang: 16pt)

#set list(marker: text(fill: accent, weight: "bold")[–], indent: 10pt,
  spacing: 0.45em, body-indent: 6pt)

// header: name + subtitle left, contacts on two left-aligned lines below
#block(above: 0pt, below: 12pt)[
  #text(font: dispf, size: 23pt, weight: "bold")[Firstname Lastname]
  #v(-3pt)
  #text(size: 10.5pt)[Backend Software Developer — B.Sc. in Computer Science]
  #v(-4pt)
  #text(size: 9pt, fill: muted)[
    #nb(mk.pin)[City, Region] #h(5pt)·#h(5pt) #nb(mk.phone)[#link("tel:+15555550100")[(555) 555-0100]]
